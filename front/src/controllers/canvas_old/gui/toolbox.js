...
class Toolbox {
    ...
    loadSavedGates() {
        SaveManager.loadSavedGatesFromFile(this.axios, this.setupSavedGates.bind(this));
    }
    ...
    setupSavedGates(savedGatesData) {
        ...
        savedGatesData.forEach((gateObj, index) => {
            const savedGate = new SavedGate(this.ctx, this.axios, this, gateObj.id, gateObj.name, gateObj.logicFunction, gateObj.ios);
            savedGate.start();
